"use client";
import { offerProductType } from "@/interFace/apiInterFace";
...
const OfeerProductList = () => {
  const { header, user } = useGlobalContext();
  const [products, setProducts] = useState<offerProductType[]>([]);
  ...
  const handleDeleteProduct = (id: string) => {
    if (user?.email) {
      axios
        .delete(
          `${process.env.BASE_URL}product/delete-offer-product/${id}?email=${user?.email}`,
          header
        )
        .then((res) => {
  ...
  const handleInputChange = (e: any) => {
    ...
    axios
      .get(
        `${process.env.BASE_URL}product/search-offer-products?search=${searchValue}`
      )
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.BASE_URL}product/all-offer-products?page=${page}&limit=${limit}`
      )
      .then((res) => {
                  ...
                  <div className="cashier-salereturns-table-inner-wrapperD border border-solid border-grayBorder border-b-0 mb-7">
                    ...
                    {products.map((item) => (
                      ...
                      >
                        ...
                        <div className="cashier-salereturns-table-companyF">
                          <span><span className="status-tag text-[12px] ml-5 font-semibold leading-[20px] text-white px-2.5 h-5 rounded-[3px] inline-block bg-themeWarn">{item?.offerPersent}%</span></span>
                        </div>
                          ...
                          <div className="dropdown">
                            ...
                            >
                              <button className="dropdown-menu-item">
                                ...
                                <Link
                                  href={`/product/product-featured/${item.productId}`}
                                >
                                  Edit
                                </Link>
                              </button>
...
export default OfeerProductList;
