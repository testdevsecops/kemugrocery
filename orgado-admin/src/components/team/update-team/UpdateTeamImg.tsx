...
const UpdateTeamImgOne = ({ myproduct, closeModal, setchangeImage }: any) => {
  ...
  const updateInfo = {
    id: myproduct._id,
    img: imgUpload,
    imgTwo: myproduct?.imgTwo,
    imgThree: myproduct?.imgThree,
  };
  const handleUpdate = () => {
    axios
      .put(
        `${process.env.BASE_URL}team/update-banner?email=${user?.email}`,
        updateInfo,
        header
      )
      .then((res) => {
...
export default UpdateTeamImgOne;
